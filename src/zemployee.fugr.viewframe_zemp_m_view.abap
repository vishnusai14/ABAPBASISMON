*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZEMP_M_VIEW
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZEMP_M_VIEW         .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZEMP_M_VIEW'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZEMP_M_VIEW_EXTRACT
TOTAL = ZEMP_M_VIEW_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZEMP_M_VIEW'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZEMP_M_VIEW-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZEMP_M_VIEW_EXTRACT
TOTAL = ZEMP_M_VIEW_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZEMP_M_VIEW-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZEMP_M_VIEW'
...
TOTAL = ZEMP_M_VIEW_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZEMP_M_VIEW-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZEMP_M_VIEW-UPD_FLAG NE SPACE.
